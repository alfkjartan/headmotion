...
public class Plotter extends JFrame implements Printable, ActionListener {
	...
	if (returnVal == JFileChooser.APPROVE_OPTION) {
		...
		for (int i=0;i<theData.length;i++) {
		    ...
		    int nseries = thePlots[i].getNumDataSets();
		    String[] headers = new String[nseries];
		    
		    
		    for (int s=0; s < nseries; s++) {
			String legend = thePlots[i].getLegend(s);
			if (legend == null) {
		    ...
		    int j = 0;
		    for (Enumeration enum = theData[i].elements();
			 enum.hasMoreElements();) {
			DoubleMatrix1D x = 
			    (DoubleMatrix1D) enum.nextElement();
			DoubleMatrix1D y = 
			    (DoubleMatrix1D) enum.nextElement();
			...
			for (int row=0; row < x.size(); row++) {
			    out.print(x.getQuick(row));
			    out.println("\t" + y.getQuick(row));
			}
		    }
